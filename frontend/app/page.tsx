...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Cold start handling: show progressive messages
    ...
    try {
      ...
      // Increased timeout for HF Spaces cold starts (60 seconds)
      ...
      if (err instanceof Error && err.name === "AbortError") {
        setError("Request timed out. The server may be sleeping. Please wait a moment and try again.");
      } else {
        ...
        <div className="max-w-3xl mx-auto px-6 py-8">
          <div className="flex flex-col items-center gap-4 text-sm text-slate-500">
            <div className="flex items-center gap-2">
              ...
              <span> time and effort by</span>
              <span className="font-semibold text-slate-700">Fazlul Karim</span>
            </div>
            ...
            <p className="text-xs text-slate-400 mt-2">
              Built with the DASS-42 questionnaire · Not a substitute for professional diagnosis
            </p>
          </div>
        </div>
      </footer>
